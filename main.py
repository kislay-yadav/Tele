...
async def _setup_channel_listener(c: TelegramClient, channel: str):
    """Set up handler to catch channel replies for resolve requests."""
    SKIP = ['arolinks.com','mahnokari.com','t.co','bit.ly']

    @c.on(events.NewMessage(chats=channel))
    async def _on_channel(event):
        text = event.message.text or ""
        urls = re.findall(r'https?://\S+', text)
        dest_urls = [u.rstrip('.,)') for u in urls if not any(s in u for s in SKIP)]
        if not dest_urls:
            return
        # Match by req_id tag or code
        matched = False
        ...
        if not matched and _resolve_pending:
            oldest = next(iter(_resolve_pending))
            fut = _resolve_pending[oldest]
            if not fut.done():
                fut.set_result(dest_urls[0])
...
async def resolve_arolink(req):
    ...
    try:
        ...
        url     = data.get("url","")
        channel = data.get("channel", os.environ.get("TG_CHANNEL",""))
        ...
        if not channel:
            return web.json_response({"ok":False,"error":"No TG_CHANNEL configured"})

        c = await _get_owner_client()
        if not c:
            return web.json_response({"ok":False,"error":"No active userbot session. Register at /register first."})
        ...
        # Set up listener (safe to call multiple times — telethon deduplicates)
        await _setup_channel_listener(c, channel)
        ...
        try:
            ...
            return web.json_response({"ok":False,"error":"Channel didn't reply in 30s","code":code})
        finally:
