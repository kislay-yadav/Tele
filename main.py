...
# ═══════════════════════════════════════════════════════
#         AROLINK CATCHER — resolve + get-session
# ═══════════════════════════════════════════════════════

# pending resolve requests: req_id → Future
_resolve_pending : dict[str, asyncio.Future] = {}
_resolve_codes   : dict[str, str]            = {}   # req_id → arolink code
_owner_client    : TelegramClient | None     = None  # owner's userbot client

async def _get_owner_client() -> TelegramClient | None:
    """Return the owner's TelegramClient from active_clients, or None."""
    global _owner_client
    # Try to find owner's session in db (first user with OWNER_ID phone, or owner_session)
    if _owner_client and _owner_client.is_connected():
        return _owner_client
    # Look for owner session stored in db
    ...
    if owner_sess:
        try:
            ...
            pass
    # Fall back: use any active client
    for slug, c in active_clients.items():
        if "_p" not in slug:
            try:
                if await c.is_user_authorized():
                    _owner_client = c
...
# Track which channels we've already registered a listener for
_channel_listeners_registered = set()

def _register_channel_listener(c: TelegramClient, channel: str):
    """Register channel listener ONCE per channel. Telethon-safe."""
    key = f"{id(c)}:{channel}"
    if key in _channel_listeners_registered:
        return
    _channel_listeners_registered.add(key)

    SKIP = ['arolinks.com', 'mahnokari.com', 't.co', 'bit.ly',
            'google.com', 'telegram.org', 'telegra.ph']

    @c.on(events.NewMessage())
    async def _on_any_message(event):
        """Listen to ALL messages — filter by channel inside handler."""
        try:
            chat = await event.get_chat()
            chat_username = getattr(chat, 'username', '') or ''
            chat_id = getattr(chat, 'id', 0)

            # Check if this message is from our target channel
            target = channel.lstrip('@').lower()
            is_target = (
                chat_username.lower() == target or
                str(chat_id) == target or
                str(chat_id) == f"-100{target}"
            )
            if not is_target:
                return

            text = event.message.text or ""
            logging.info(f"[AroLink] Channel msg: {text[:120]}")
            ...
            for u in urls:
                u = u.rstrip('.,);')
                if not any(s in u for s in SKIP) and len(u) > 15:
                    dest_urls.append(u)

            if not dest_urls and not _resolve_pending:
                return

            # If no dest URLs found but there are pending requests,
            # pass the raw text — maybe the URL is the whole reply
            if not dest_urls:
                # Try the whole message as a URL
                stripped = text.strip()
            ...
            result_url = dest_urls[0]

            # Try to match by req_id tag in the channel message
            matched = False
            for req_id, code in list(_resolve_codes.items()):
                if f"#req_{req_id}" in text or code in text:
                    fut = _resolve_pending.get(req_id)
                    if fut and not fut.done():
                        fut.set_result(result_url)
                        matched = True
                        logging.info(f"[AroLink] ✅ Matched req {req_id} → {result_url}")
                    break

            # Fallback: resolve ALL pending (channel replied = one request active)
            if not matched and _resolve_pending:
                for req_id, fut in list(_resolve_pending.items()):
                    if not fut.done():
                        fut.set_result(result_url)
                        logging.info(f"[AroLink] ✅ Fallback resolve {req_id} → {result_url}")

        except Exception as e:
            logging.error(f"[AroLink] Channel handler error: {e}")

async def resolve_arolink(req):
    """POST /resolve  {url, secret, channel}"""
    try:
        data    = await req.json()
        secret  = data.get("secret", "")
        url     = data.get("url", "")
        channel = data.get("channel", "") or os.environ.get("TG_CHANNEL", "")

        # Auth check
        expected = os.environ.get("SECRET_KEY", cfg("secret_key", "changeme"))
        if secret != expected:
            return web.json_response({"ok": False, "error": "Unauthorized"}, status=401)

        if "arolinks.com" not in url:
            return web.json_response({"ok": False, "error": "Not an arolinks URL"})

        if not channel:
            return web.json_response({"ok": False, "error": "TG_CHANNEL not set in env vars"})

        c = await _get_owner_client()
        if not c:
            return web.json_response({"ok": False, "error": "No userbot session — register at /register"})

        # Register channel listener ONCE
        _register_channel_listener(c, channel)
        ...
        _resolve_pending[req_id] = fut
        _resolve_codes[req_id]   = code

        # Send arolink to channel
        await c.send_message(channel, f"🔗 {url}\n#req_{req_id}")
        logging.info(f"[AroLink] 📤 Sent to channel: {url}")

        # Wait indefinitely — no timeout
        # aiohttp will keep the connection open until result arrives
        result = await fut

        return web.json_response({"ok": True, "url": result, "code": code})

    except Exception as e:
        logging.error(f"[AroLink] resolve error: {e}")
        return web.json_response({"ok": False, "error": str(e)})
    finally:
        _resolve_pending.pop(req_id, None)
        _resolve_codes.pop(req_id, None)

async def get_session(req):
    """
    GET /get-session?secret=xxx&phone=+91xxxxxxxxxx
    OR  GET /get-session?secret=xxx&slug=yourslug
    Returns the session string for your account so you can use it elsewhere.
    OWNER ONLY — protected by SECRET_KEY.
    """
    secret   = req.rel_url.query.get("secret","")
    expected = os.environ.get("SECRET_KEY", cfg("secret_key","changeme"))
    if secret != expected:
        return web.Response(text="❌ Unauthorized", status=401)

    phone = req.rel_url.query.get("phone","")
    slug  = req.rel_url.query.get("slug","")

    # Find by slug
    if slug and slug in db["users"]:
        ss = db["users"][slug].get("session","")
        if ss:
            return web.json_response({"ok":True,"slug":slug,"session":ss,
                                      "name":db["users"][slug].get("name","")})

    # Find by phone
    if phone:
        phone_clean = phone.replace(" ","").replace("-","")
        for s, u in db["users"].items():
            if u.get("phone","").replace(" ","") == phone_clean or \
               ("+" + u.get("phone","")) == phone_clean:
                return web.json_response({"ok":True,"slug":s,"session":u["session"],
                                          "name":u.get("name","")})

    # List all users (so owner can pick)
    users_list = [{"slug":s,"name":u["name"],"phone":u.get("phone","")}
                  for s,u in db["users"].items()]
    return web.json_response({"ok":False,
        "error":"User not found. Use ?slug= or ?phone=",
...
def build_web_app():
    ...
    app.router.add_get("/get-session",   get_session)   # ← NEW
    app.router.add_post("/resolve",      resolve_arolink) # ← NEW
